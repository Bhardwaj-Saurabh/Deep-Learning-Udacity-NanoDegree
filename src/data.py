...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose([
                                    ...
                                    transforms.RandomResizedCrop(crop_size),  # Randomly crop to 224x224
                                    transforms.RandomHorizontalFlip(),      # Randomly flip horizontally
                                    transforms.RandomRotation(30),          # Randomly rotate by up to 30 degrees
                                    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2),  # Random color jitter
                                    transforms.ToTensor(),                  # Convert to tensor
                                    transforms.Normalize(mean, std)  # Normalize with ImageNet statistics
                                ]
        ),
        "valid": transforms.Compose([
                                    transforms.Resize(image_size),       # Resize to 256x256
                                    transforms.CenterCrop(crop_size),    # Crop to center 224x224
                                    transforms.ToTensor(),               # Convert to tensor
                                    transforms.Normalize(mean, std)  # Normalize with ImageNet statistics
                                ]
        ),
    ...
    train_data = datasets.ImageFolder(
        ...
        transform=data_transforms["train"]  # Use the "train" transform
    )
    ...
    valid_data = datasets.ImageFolder(
        ...
        # add the appropriate transform that you defined in
        # the data_transforms dictionary
        transform=data_transforms["valid"]  # Use the "train" transform
    )
